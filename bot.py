...
try:
    ...
    # Важно: если здесь произошла ошибка, бот не сможет работать с Google Sheets.
    # Поэтому мы поднимаем исключение, чтобы остановить развертывание, пока проблема не будет решена.
    raise
...
CHANNELS = [
    "-1002657330561", # Пример ID канала 1
    "-1002243633174", # Пример ID канала 2
    "-1002484534545", # Пример ID канала 3
    "-1002578865225", # Пример ID канала 4
    "-1002617434713", # Пример ID канала 5
]

# URL-адреса для кнопок подписки (лучше хранить их динамически, например, в той же таблице Sheets)
# Убедитесь, что эти ссылки актуальны и ведут на ваши каналы.
CHANNEL_BUTTONS = [
...
def check_subscription(update: Update, context: CallbackContext) -> None:
    ...
    for channel_id in CHANNELS:
        try:
            ...
            if member.status not in ["member", "administrator", "creator"]:
                ...
                # failed_channel = channel_id # Для отладки можно запоминать, на какой канал не подписан
                break
        except Exception as e:
            logger.error(f"Ошибка при проверке подписки на канал {channel_id}: {e}")
            # Если бот не может проверить подписку, считаем, что не подписан
            all_subscribed = False
            # failed_channel = channel_id # Для отладки
            break
    ...
    if all_subscribed:
        ...
        # Если нужно указать конкретный канал, используйте failed_channel
        # if failed_channel:
        #     error_message += f" (Проблема с каналом ID: `{failed_channel}`)." # Для отладки
...
def find_movie_by_code(code: str) -> dict:
    ...
    try:
        ...
        for row in data:
            ...
            # Имя колонки "Название" должно совпадать с заголовком в вашей таблице
