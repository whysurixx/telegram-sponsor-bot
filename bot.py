    ...
    if update.message.text.startswith("/start invite_"):
        try:
            ...
            if referrer_id == user_id:
                ...
                await send_message_with_retry(update.message, "❌ Вы не можете пригласить себя!", reply_markup=get_main_keyboard())
                return
            else:
            ...
            if referrer_data:
                ...
                try:
                    await bot.send_message(
                        chat_id=referrer_id,
                        text=f"🎉 Поздравляем! Ваш друг (ID: {user_id}) подписался на каналы и начал использовать бота. Вам начислено *+2 поиска*!",
                        parse_mode='Markdown'
                    )
...
async def handle_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.message and update.message.from_user:
        ...
        if text == "🔍 Поиск фильма":
            ...
            referral_text = (
    		"🔥 *Реферальная система* 🔥\n\n"
    		"Приглашай друзей и получай *+2 поиска* за каждого, кто перейдёт по твоей ссылке и подпишется на все наши каналы! 🎉\n"
    		"Как это работает:\n"
    		"1. Скопируй свою реферальную ссылку: `{referral_link}`\n"
    		"2. Отправь её другу.\n"
    		"3. Друг должен подписаться на все каналы и нажать *Я ПОДПИСАЛСЯ!* 😎\n"
    		"4. После этого тебе начислят *+2 поиска*! 🚀\n\n"
    		f"👥 *Количество приглашённых друзей*: *{invited_users}*\n"
    		f"🔍 *Количество оставшихся поисков*: *{search_queries}*"
	 	)
